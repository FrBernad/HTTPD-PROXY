...
await_doh_response_on_read_ready(struct selector_key *key) {
    ...
    while (buffer_can_read(origin_buffer)) {
        if (connection->client.doh.statusLineParser.state != status_line_done) {
            status_line_state statusLineState = status_line_parser_feed(&connection->client.doh.statusLineParser,buffer_read(origin_buffer));
        ...
        if (connection->client.doh.headersParser.state != headers_done) {
            headers_state headersState = headers_parser_feed(&connection->client.doh.headersParser,buffer_read(origin_buffer));
        ...
        if (connection->client.doh.dohParser.state != doh_response_done) {
            doh_response_state dohResponseState = doh_response_parser_feed(&connection->client.doh.dohParser,buffer_read(origin_buffer));
